...
public class CommonObjects {
    ...
    public void waitUntilFinishedLoading() {
        webDriverWait.until((driver) ->
                getJSObject().executeScript("return document.readyState").toString().equals("complete")
        );
    }
    ...
    /**
     * Selects dropdown value by visible text (ignoring case)
     *
     * @param dropdownElement
     * @param fieldValue
     */
    public void selectDropdownValue(WebElement dropdownElement, String fieldValue) {
        waitElementVisibility(dropdownElement);
        scrollIntoView(dropdownElement, 200);
        Select dropDown = new Select(dropdownElement);
        int index = 0;
        for (WebElement option : dropDown.getOptions()) {
            String optionText = option.getText().trim().replaceAll("\\s+", " ");
            if (optionText.equalsIgnoreCase(fieldValue.trim().replaceAll("\\s+", " "))) {
                break;
            }
            index++;
        }
    ...
    public List<String> getDropdownValues(WebElement webElement) {
        WebElement dropdownElement = getElement(webElement);
        scrollIntoView(dropdownElement);
        List<String> dropdownValues = new ArrayList<>();
        Select dropDown = new Select(dropdownElement);
        for (WebElement option : dropDown.getOptions()) {
            String optionText = getText(option).replaceAll("\\s+", " ");
            dropdownValues.add(optionText);
        }
